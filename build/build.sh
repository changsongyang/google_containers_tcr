...
case "$1" in
  ...
  "docker") #使用容器编译和打包dist
    docker build -t ccr.ccs.tencentyun.com/registry.ycsit.cn/google_containers_imgsync:$TAG_NUM $build_arg \
      --build-arg LDFLAGS="${LDFLAGS}" -f ${Dockerfile:=Dockerfile} .
    [ -n "${DockerUser}" ] && {
      docker login -u "${DockerUser}" "${DockerPass}"
      docker push ccr.ccs.tencentyun.com/registry.ycsit.cn/google_containers_imgsync:$TAG_NUM
    }
    ;;
  "clean")
